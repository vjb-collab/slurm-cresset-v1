...
def install_packages():
    ...
    add_mon_url = f'https://storage.googleapis.com/hpc-sandbox-slurm-install-scripts/{add_mon_script.name}'
    urllib.request.urlretrieve(add_mon_url, add_mon_script)
    util.run(f"bash {add_mon_script}")
    util.run("yum install -y --nogpgcheck stackdriver-agent")

    add_log_script = Path('/tmp/install-logging-agent.sh')
    add_log_url = f'https://storage.googleapis.com/hpc-sandbox-slurm-install-scripts/{add_log_script.name}'
